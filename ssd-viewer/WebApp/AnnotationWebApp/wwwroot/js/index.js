...
Paint.prototype.saveRect = function (){
    ...
    if (this.prevRect) {
        ...
        const lastRect = this.currentRects[this.currentRects.length - 1];
        //in case if user just click on the canvas, without dragging
        if (lastRect) {
            if (lastRect.startX !== this.prevRect.startX && lastRect.endX !== this.prevRect.endX
                && lastRect.width !== this.prevRect.width && lastRect.height !== this.prevRect.height
                && lastRect.startY !== this.prevRect.startY && lastRect.endY !== this.prevRect.endY) {
...
send.addEventListener('submit', e => {
    ...
    if (rects.length > 0) {
        const jsonRects = JSON.stringify(rects);
        const input = send.querySelector(`#tumorPosition`);
        input.value = jsonRects;
        send.submit();
    } else {
        alert("Please select the area of images.")
    }
})
